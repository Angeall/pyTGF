...
class TestSokoban(unittest.TestCase):
    def setUp(self):
        ...
        self.controller = list(self.game._controllers.keys())[0]
        if isinstance(self.game._controllers[self.controller], Box):
            self.controller = list(self.game._controllers.keys())[1]

    def test_push_box_in_hole(self):
        self.game._winningTiles.append((1, 0))
        self.controller.moves.put((1, 0))
        self.assertTrue(self.game.board.getTileById((2, 0)).deadly)
        self.game.run(60)
        box = None
        for unit in self.game._controllers.values():
            if isinstance(unit, Box):
                box = unit
        self.assertTrue(box is not None)
        self.assertEqual(self.game._units[box], (2, 0))
        self.assertFalse(self.game.board.getTileById((2, 0)).deadly)
